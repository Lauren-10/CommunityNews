...
def scraper_inner_loop(df,schema,tags_to_extract,table_name, llm):
    db_admin = MySQLConnector('wthomps3', permission = 'admin')
...
def chatgpt_to_sql(df,chunk_size,llm,schema,prompts,tags_to_extract,table_name = 'student_journalists23_24'):
    ...
    for df in list_df:
        scraper_inner_loop(df,schema,prompts,tags_to_extract,table_name)
